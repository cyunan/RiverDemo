...
public class MainScene extends Fragment implements View.OnClickListener {
    ...
    private void initView(View view) {
        ...
        btnFbLogin = view.findViewById(ResourceUtils.getResourceId(activity, "btn_fb_login"));
        btnTrackEvent = view.findViewById(ResourceUtils.getResourceId(activity, "btn_track_event"));

        btnFbFriends = view.findViewById(ResourceUtils.getResourceId(activity, "btn_fb_friends"));
        ...
        btnLogout = view.findViewById(ResourceUtils.getResourceId(activity, "btn_logout"));
        ...
        btnFbFriends.setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (v.equals(btnSetting)) {
            ...
            billing();
        } else if (v.equals(btnFbLogin)) {
            fbLogin();
        } else if (v.equals(btnTrackEvent)) {
            trackGameEvent();
        } else if (v.equals(btnFbFriends)) {
            getFbFriends();
    ...
    private void logout() {
        riverSDKApi.sqSDKLogout(activity, new SDKCallback() {
            @Override
            public void onResult(int statusCode, Map<String, String> params) {
                if (statusCode == StatusCode.SUCCESS) {
                    // 这里会收到游客登录成功的回调
                    // TODO: 2024/2/19 如果游客账号和fb账号不属于一个平台账号，需要重进游戏页面
                } else {
                    //登出失败,获取提示信息
                    String msg = params.get(CallbackKey.MSG);
                    Log.i(TAG, "logout failed: " + msg);
                }
            }
        });
    ...
    private void getFbFriends() {
        riverSDKApi.sqSDKFBInGameFriendInfo(activity, new SDKCallback() {
            @Override
            public void onResult(int statusCode, Map<String, String> params) {
                if (StatusCode.SUCCESS == statusCode) {
                    //我的好友信息
                    String friendsInfo = params.get(CallbackKey.FB_FRIENDS_INFO);
                    //我的fbid
                    String myFbId = params.get(CallbackKey.FB_ID);
                    //我的accessToken
                    String myAccessToken = params.get(CallbackKey.FB_TOKEN);
                    //我的fb名称
                    String myFbName = params.get(CallbackKey.FB_NAME);
                    //我的fb头像
                    String myFbPicture = params.get(CallbackKey.FB_PICTURE);
                    String msg = params.get(CallbackKey.MSG);
                    Log.i(TAG, "sqSDKGetFBFriendInfo onResult " +
                            " msg=" + msg + " friendsInfo=" + friendsInfo
                            + "myFbId=" + myFbId + " myFbName=" + myFbName +
                            " myFbPicture=" + myFbPicture);
                }
            }
        });
    }
    ...
    /**
     * 游戏内需要设置一个fb登录的按钮，然后调用fb登录接口
     * 如果游客是新账号，fb登录成功会直接绑定到平台账号上
     * 如果游客是老账号，fb登录成功会直接切换到原平台账号
     */
    private void fbLogin() {
        riverSDKApi.sqSDKPlatformLogin(activity, UserType.FACEBOOK_TYPE, new SDKCallback() {
            @Override
            public void onResult(int statusCode, Map<String, String> params) {
                if (StatusCode.SUCCESS == statusCode) {

                    // TODO: 2024/2/19 这里如果和上次登陆的游客账号不一样，需要重新进入游戏页面
    ...
    private void billing() {
        activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                progressDialog.setMessage("Loading..."); // 设置loading提示信息
                progressDialog.setCancelable(false); // 设置是否可以通过返回键取消dialog
